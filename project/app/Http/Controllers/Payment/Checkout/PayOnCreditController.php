...
class PayOnCreditController extends CheckoutBaseControlller
{
    ...
    private function createOrder(array &$input, Cart $cart, float $orderTotal, ?string $affilateUsers, Request $request): Order
    {
        ...
        $input['credit_pay_time'] = !empty($input['payment_time']) ? $input['payment_time'] : 'advance';
        ...
        $cashbackPercentage = Cashback::getCashbackPercentage($orderTotal, $input['payment_time']);
    ...
    private function creditWalletWithCashbackTransaction(array $input, $order, $orderCalculate)
    {
        ...
        $creditTime = $order['credit_pay_time'] ?? "advance";
        $cashbackPercentage = Cashback::getCashbackPercentage($orderCalculate['total_amount'], $creditTime);
    ...
    private function redeemCashbackTransaction(array $input, $order, $orderCalculate)
    {
        ...
        $deposit->method = $order->order_number;
        $deposit->txnid = $order->order_number;
        $deposit->details = "Cashback redeemed for the Order Number : " . $order['order_number'] . ". Adjusted in the Invoice.";
        $deposit->type = "debit";
        $deposit->flutter_id = null;
        $deposit->status = 1;
        $deposit->save();
