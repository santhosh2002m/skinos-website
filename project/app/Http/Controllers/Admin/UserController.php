    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ->addColumn('action', function(User $data) {
                ...
                $details = '<a href="' . route('admin-user-show', $data->id) . '" >
                                <i class="fas fa-eye"></i> '.__("Details").'
                            </a>';
                $newLogin = '<a href="javascript:;" class="login-as-user-btn" data-user-id="' .  $data->id  .'">
                ...
                if ($user && $user->role_id == 0) {
                    ...
                    $vendor = $data->is_vendor != 2 
                        ? '<a href="javascript:;" data-toggle="modal" data-target="#modal1" class="make-vendor" data-href="' . route('admin-user-vendor',$data->id) . '" > 
                    ...
                    return '<div class="action-list">'
                            .'<a href="javascript:;" data-toggle="modal" data-target="#modal1" class="deposit" data-href="' . route('admin-user-deposit',$data->id) . '" > 
                                <i class="fas fa-rupee-sign"></i> '.__("Manage Wallet").'
                            </a>'
                            .$details
...
// OTHER SECTION ENDS
    ...
    public function depositupdate(Request $request, $id)
    {
        ...
        $deposit->method = $request->method ?? null;
        $deposit->details = $request->details ?? null;
        $deposit->type = $request->type ?? null;
        $deposit->txnid = $request->txnid ?? null;
        $deposit->flutter_id = null;
        $deposit->status = 1;
        $deposit->save();
        return response()->json(__('Balance Updated Successfully.'));
    }
    ...
    public function sendLoginOtp($id)
    {
        $user = User::findOrFail($id);
