...
@section('content')
    <style>
        ...
        .template-btn.btn-forms:hover {
            background-color: #ff851b !important;
        }

        /* Hover effect for the Password Visibility Icons */
        .pass-wrapper .eye-off,
        .pass-wrapper .confirm-eye-off {
            transition: fill 0.3s ease;
        }

        .pass-wrapper .eye-off:hover,
        .pass-wrapper .confirm-eye-off:hover {
            fill: #ff851b !important;
        }

        .pass-wrapper .eye-on,
        .pass-wrapper .confirm-eye-on {
            transition: stroke 0.3s ease;
        }

        .pass-wrapper .eye-on:hover,
        .pass-wrapper .confirm-eye-on:hover {
        ...
        /* Custom styling for dropdowns to match the image */
        .form-group select {
            -webkit-appearance: none;
            -moz-appearance: none;
            appearance: none;
            background: #fff url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' viewBox='0 0 16 16'><path d='M4 6l4 4 4-4z' fill='%23000000'/></svg>") no-repeat right 10px center;
        ...
        .form-group select:focus {
            outline: none;
            border-color: #ff851b;
            box-shadow: 0 0 5px rgba(255, 133, 27, 0.5);
        }
    </style>
    ...
    <section class="gs-reg-section wow-replaced" data-wow-delay=".1s">
        <div class="container">
            <div class="row">
                <div class="col-lg-6 mx-auto reg-area mt-5 rounded">
                    <div class="reg-content">
                        <h4 class="text-center">@lang('Create Your New Account')</h4>
                        <form action="{{ route('user-register-submit') }}" method="POST" id="register-form">
                            @csrf
                            ...
                            <div class="form-group">
                                <label for="zip">@lang('Postal Code')</label>
                                <input type="text" class="form-control" id="zip" name="zip"
                                    placeholder="@lang('Enter your postal code')">
                                @error('zip')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group select_state d-none">
                                <label for="show_state">@lang('Select State')</label>
                                <select class="form-control" id="show_state" name="state">
                            ...
                            <div class="form-group d-none">
                                <label for="show_city">@lang('Select City')</label>
                                <select class="form-control" id="show_city" name="city">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="hcp_clinic_name" name="hcp_clinic_name"
                                    placeholder="@lang('Enter clinic name')">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="doctor_name" name="doctor_name"
                                    placeholder="@lang('Enter doctor name')">
                                @error('doctor_name')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="preferred_type" name="preferred_type">
                                    <option value="" disabled selected>@lang('Select Preferred Type')</option>
                                    <option value="scheme_based_profile">@lang('Scheme based Profile')</option>
                                    <option value="net_discount_profile">@lang('Net Discount Profile')</option>
                                </select>
                                ...
                                <div class="pass-wrapper">
                                    ...
                                    <svg class="eye-on" xmlns="http://www.w3.org/2000/svg" width="18" height="14"
                                        ...
                                        <path
                                            ...
                                            stroke="#001f3f" stroke-width="1.33333" stroke-linecap="round"
                            ...
                            <div class="form-group">
                                <button type="submit" class="template-btn btn-forms"
                                    style="background-color: #001f3f;">@lang('Register')</button>
                                <p class="login-redirect">@lang('Already have an account?') <span><a
                                            href="{{ route('user.login') }}"
                                            style="color: #001f3f;">@lang('Login')</a></span></p>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change', '#select_country', function() {
                let country_id = $(this).find('option:selected').attr('data');
                let state_url = "{{ route('country.wise.state', '') }}/" + country_id;
                ...
                if (country_id) {
                    $.get(state_url, function(response) {
                        if (response.data) {
                            ...
                            // Trigger state change if a state is pre-selected
                            if (response.state) {
                                $('#show_state').val(response.state).trigger('change');
                            }
                        } else {
                            $('.select_state').addClass('d-none');
                            $('#show_city').closest('.form-group').addClass('d-none');
                        }
                    });
                } else {
                    $('.select_state').addClass('d-none');
            ...
            $(document).on('change', '#show_state', function() {
                ...
                let city_url = "{{ route('state.wise.city') }}";

                if (state_id) {
                    $.get(city_url, { state_id: state_id }, function(data) {
                        if (data.data) {
                            $('#show_city').closest('.form-group').removeClass('d-none');
            ...
            $('#register-form').on('submit', function(e) {
                ...
                const stateVisible = $('.select_state').is(':visible');
                const cityVisible = $('#show_city').closest('.form-group').is(':visible');
            ...
            $('.pass-wrapper .eye-off, .pass-wrapper .eye-on').click(function() {
                let input = $(this).siblings('input');
                let eyeOff = $(this).hasClass('eye-off') ? $(this) : $(this).siblings('.eye-off');
                let eyeOn = $(this).hasClass('eye-on') ? $(this) : $(this).siblings('.eye-on');
            ...
            $('.pass-wrapper .confirm-eye-off, .pass-wrapper .confirm-eye-on').click(function() {
                let input = $(this).siblings('input');
                let eyeOff = $(this).hasClass('confirm-eye-off') ? $(this) : $(this).siblings('.confirm-eye-off');
                let eyeOn = $(this).hasClass('confirm-eye-on') ? $(this) : $(this).siblings('.confirm-eye-on');
