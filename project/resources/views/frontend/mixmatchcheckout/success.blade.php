                            ...
                            <tbody>
                                ...
                                <tr class="wow-replaced" data-wow-delay=".1s">
                                    ...
                                    <td class="product-details">
                                        <img src="{{ asset('assets/images/products/' . $product->item->photo) }}" alt="" class="d-lg-none d-table-cell pb-24 small-device-img">
                                        ...
                                        {{-- @if (!empty($product->size))
                                            <p><span>Size:</span>
                                                <b>{{ __('Size') }}</b>: {{ $product->item->measure }}{{ str_replace('-', ' ', $product->size) }}
                                            </p>
                                        @endif --}}
                                        ...
                                        {{-- @if (!empty($product->color))
                                            <p><span>@lang('Color:')</span>
                                                <div class="d-flex mt-2">
                                                    <b>{{ __('Color') }}</b>: 
                                                    <span class="color-show-btn mt-1 ms-3" style="background-color: #{{ $product->color ?: 'ffffff' }};"></span>
                                                </div>
                                            </p>
                                        @endif --}}
                            
                                        {{-- Custom Attributes (Keys/Values) --}}
                                        @if (!empty($product->keys))
                                            @foreach (array_combine(explode(',', $product->keys), explode(',', $product->values)) as $key => $value)
                                                <p><span>{{ ucwords(str_replace('_', ' ', $key)) }}:</span> {{ $value }}</p>
                                            @endforeach
                                        @endif
                            
                                        {{-- Mobile Unit Price and Total Price --}}
                                        ...
                                        <p class="d-lg-none"><span>@lang('Total Price:')</span>
                                            {{ \PriceHelper::showCurrencyPrice($product->price * $order->currency_value) }}
                                            <small>{{ $product->discount ? '(' . $product->discount . '% ' . __('Off') . ')' : '' }}</small>
                                        </p>
                                    </td>
