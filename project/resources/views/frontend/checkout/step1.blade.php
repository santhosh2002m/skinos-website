...
@section('content')
    ...
    <div class="gs-checkout-wrapper">
        <div class="container">
            ...
            <!-- address-->
            <form id="checkout-form" class="address-wrapper" action="{{ route('front.checkout.step1.submit') }}"
                method="POST">
                @csrf
                <div class="row gy-4">
                            ...
                            <div class="row g-4">
                                <div class="col-lg-6 d-none">
                                    <div class="input-wrapper">
                                        ...
                                        <select id="shipop" name="shipping">
                        ...
                        <div class="collapse" id="show_shipping_address">
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-lg-6">
                                    <div class="input-wrapper">
                                        <label class="label-cls">@lang('Select Country')</label>
                                        <select class="nice-select" id="shipping_country" name="shipping_country">
                                            @include('includes.shipcountries')
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-6 d-none select_shipping_state">
                                    <div class="input-wrapper">
                                        <label class="label-cls">@lang('Select State')</label>
                                        <select class="nice-select" id="shipping_state" name="shipping_state">
                                            <option value="">@lang('Select State')</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-6 d-none select_shipping_city">
                                    <div class="input-wrapper">
                                        <label class="label-cls">@lang('Select City')</label>
                                        <select class="nice-select" id="shipping_city" name="shipping_city">
    ...
    <script type="text/javascript">
        ...
        $(document).on('change', '#shipping_country', function() {
            ...
            if (country_id) {
                $.get(state_url, function(response) {
                    if (response.data) {
                        $('.select_shipping_state').removeClass('d-none');
                        $('#shipping_state').html(response.data);
                        if (response.state) {
                            $('#shipping_state').val(response.state).trigger('change');
                        }
                    } else {
                        $('.select_shipping_state').addClass('d-none');
                        $('.select_shipping_city').addClass('d-none');
                    }
                    $('#shipping_state').niceSelect();
                });
            } else {
                $('.select_shipping_state').addClass('d-none');
                $('.select_shipping_city').addClass('d-none');
        ...
        $(document).on('change', '#shipping_state', function() {
            $('#shipping_city').niceSelect("destroy");
            let state_id = $(this).val();
            ...
            if (state_id) {
                $.get(city_url, { state_id: state_id }, function(data) {
                    if (data.data) {
                        $('.select_shipping_city').removeClass('d-none');
                        $('#shipping_city').html(data.data);
                    } else {
                        $('.select_shipping_city').addClass('d-none');
                    }
                    $('#shipping_city').niceSelect();
                });
            } else {
                $('.select_shipping_city').addClass('d-none');
            }
        ...
        $("#shpto").on("change", function() {
            if (this.checked) {
                $('#show_shipping_address input').prop('required', true);
            } else {
                $('#show_shipping_address input').prop('required', false);
            }
        ...
        $('#checkout-form').on('submit', function(e) {
            ...
            // Validate Billing Details dropdowns
            const billingCountry = $('#select_country').val();
            const billingStateVisible = $('#show_state').closest('.select_state').is(':visible');
            const billingCityVisible = $('#show_city').closest('.col-lg-6').is(':visible');

            if (!billingCountry) {
                isValid = false;
                message = "Please select a billing country.";
            } else if (billingStateVisible && !$('#show_state').val()) {
                isValid = false;
                message = "Please select a billing state.";
            } else if (billingCityVisible && !$('#show_city').val()) {
                isValid = false;
                message = "Please select a billing city.";
            }

            // Validate Shipping Address dropdowns if visible
            if ($('#shpto').is(':checked')) {
                const shippingCountry = $('#shipping_country').val();
                const shippingStateVisible = $('.select_shipping_state').is(':visible');
                const shippingCityVisible = $('.select_shipping_city').is(':visible');

                if (!shippingCountry) {
                    isValid = false;
                    message = "Please select a shipping country.";
                } else if (shippingStateVisible && !$('#shipping_state').val()) {
                    isValid = false;
                    message = "Please select a shipping state.";
                } else if (shippingCityVisible && !$('#shipping_city').val()) {
                    isValid = false;
